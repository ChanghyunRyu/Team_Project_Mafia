...
public class CMWinServer extends JFrame {
	...
	private ServerStroage storage = new ServerStroage();
	...
	public void sendRoomInfo(String sender,String group,String session) {
		// TODO Auto-generated method stub
		HashMap<Integer,Room> roomList = storage.getRoomList();
		Iterator iterator = roomList.entrySet().iterator();
		while(iterator.hasNext()) {
			Entry entry = (Entry)iterator.next();
			Room room = (Room)entry.getValue();
			CMDummyEvent de = new CMDummyEvent();
			if(room.isOn()) {break;}
			de.setDummyInfo("showRoom#" + room.getID() + "#" + room.getName()+"#"+room.getPlayerNumber());
			de.setHandlerSession(session);
			de.setHandlerGroup(group);
			m_serverStub.send(de,sender);
			de = null;
		}
	}
	public void makeRoom(String sender, String group, String session, String roomname,String nickname){
		
		int roomID = storage.makeNewRoom(roomname,sender,nickname);
		//방 생성 메시지를 보냄
		CMDummyEvent de = new CMDummyEvent();
		de.setDummyInfo("createRoom#" + roomID + "#" + roomname);
		de.setHandlerSession(session);
		de.setHandlerGroup(group);
		m_serverStub.send(de,sender);
		de = null;
	...
	public void enterRoomResult(String sender, String group, String session, int room_ID, String nickname) {
		// TODO Auto-generated method stub
		Room room  = this.storage.getRoom(room_ID);
		boolean isEnter = false;
		CMDummyEvent playerInfo = new CMDummyEvent();
		
		playerInfo.setHandlerGroup(group);
		playerInfo.setHandlerSession(session);
		playerInfo.setDummyInfo("showPlayer#"+ nickname + "#" + sender);
		
		String players = "";
		if(room.getPlayerNumber() < 8) {
			Player player = new Player(nickname,sender,false);
			room.putPlayer(player);
			isEnter = true;
			if(isEnter) {
				...
				while(iterator.hasNext()) {
					...
					players += playerdummy.getNickname()+ "," + playerdummy.getUserID() +"-";
					m_serverStub.send(playerInfo,playerdummy.getUserID());
				}
			}
		}
		String str = "enterRoom#" + isEnter + "#"+ room_ID +"#"+room.getName() +"#"+ players;
		CMDummyEvent de = new CMDummyEvent();
		de.setDummyInfo(str);
		de.setHandlerSession(session);
		de.setHandlerGroup(group);
	...
	public void gameStart(String sender, String group, String session, int room_ID) {
		// TODO Auto-generated method stub
		Room room  = this.storage.getRoom(room_ID);
		CMDummyEvent gameStartResult = new CMDummyEvent();
		...
		while(iterator.hasNext()) {
			Entry entry = (Entry)iterator.next();
			Player playerdummy = (Player)entry.getValue();
			String dummyinfo = dummyStr + "#"+playerdummy.getJob();
			gameStartResult.setDummyInfo(dummyinfo);
			printMessage("[playerdummy]:" + playerdummy.getUserID() +" ,"+ playerdummy.getNickname()+"\n" + gameStartResult.getDummyInfo());
			m_serverStub.send(gameStartResult,playerdummy.getUserID());
		}
		gameStartResult = null;
	}

	//status 0 : 밤, 1 : 낮, 2 : 투표시간, 3: 최후 변론, 4 : 찬반투표
	public void changeStatus(int status,int room_ID,int nextTime) {
		CMDummyEvent changeStatus = new CMDummyEvent();
		changeStatus.setDummyInfo("changeStatus#"+status+"#"+nextTime);
		String session ="session" + (room_ID+1);
		String group = "g1";
		changeStatus.setHandlerGroup(group);
	...
	public void castPlayerDead(int room_ID, String player_ID) {
		printMessage("[roomID: "+room_ID+"]: " + player_ID + " is Dead\n");
		Room room  = this.storage.getRoom(room_ID);
		if(!player_ID.equals("not_dead")) {
			room.getPlayer(player_ID).setLive(false);
			printMessage(room.getPlayer(player_ID).getUserID() +"is Live? :" + room.getPlayer(player_ID).isLive()+ "\n");
		}
		CMDummyEvent playerDead = new CMDummyEvent();
		playerDead.setDummyInfo("playerDead#"+ player_ID);
		String session ="session" + (room_ID+1);
		String group = "g1";
		playerDead .setHandlerGroup(group);
		playerDead .setHandlerSession(session);
		m_serverStub.cast(playerDead , session, group);
		playerDead = null;
		
		// 게임 끝났는지 판정
		String gameEnd = room.moderator.isGameEnd();
		if(!gameEnd.equals("not_end")) {
			printMessage("game end");
			CMDummyEvent endMsg = new CMDummyEvent();
			endMsg.setHandlerGroup(group);
			endMsg.setHandlerSession(session);
			endMsg.setDummyInfo("gameEnd#"+gameEnd);
			m_serverStub.cast(endMsg , session, group);
		}
	}
	
	public void put_vote(int room_ID,String target_ID) {
		Room room = this.storage.getRoom(room_ID);
		room.moderator.putVote(target_ID);
	}
	
	public void vote_result(int room_ID) {
		Room room = this.storage.getRoom(room_ID);
		String user_id = room.moderator.decision();
		castPlayerDead(room_ID, user_id);
	}
	...
	public void process_ability(String sender,String group, String session, int room_ID, int job, String user_ID) {
		...
		switch(job) {
		case 2:
			room.moderator.put_docter_target(user_ID);	
			break;
		case 3:
			result = room.moderator.resultPolice(user_ID,room);
			CMDummyEvent playerInfo = new CMDummyEvent();
			playerInfo.setHandlerGroup(group);
			playerInfo.setHandlerSession(session);
			playerInfo.setDummyInfo("ability#"+ result +"#" + user_ID+ "#" + job);
			m_serverStub.send(playerInfo, sender);
			break;
		case 4:
			room.moderator.put_mafia_target(user_ID);
			break;
		}
	}
	
	public void result_ability(int room_ID,String group, String session) {
		Room room = this.storage.getRoom(room_ID);
		castPlayerDead(room_ID, room.moderator.getMafiaResult());
		room.moderator.resetAbility();
	}
	/////////////////////////////////////////
	public int get_login_info(String id, String password) {
		return storage.UserLogin(id, password);
	}
	public void sendLoginAnounce(String user_id, String user_nickname) {
		// TODO Auto-generated method stub
		CMDummyEvent login = new CMDummyEvent();
		String group = "g1";
		String session = "session1";
		login.setHandlerSession(session);
		login.setHandlerGroup(group);
		login.setDummyInfo("login#"+user_id+"#"+user_nickname);
		m_serverStub.cast(login , session, group);
	}
	public void sendLogoutAnounce(String user_id, String user_nickname) {
		// TODO Auto-generated method stub
		CMDummyEvent logout = new CMDummyEvent();
		String group = "g1";
		String session = "session1";
		logout.setHandlerSession(session);
		logout.setHandlerGroup(group);
		logout.setDummyInfo("logout#"+user_id+"#"+user_nickname);
		m_serverStub.cast(logout , session, group);
	}
	public void castRoomout(int room_ID, String player_ID) {
		// TODO Auto-generated method stub
		Room room  = this.storage.getRoom(room_ID);
		room.removePlayer(player_ID);
		if(room.getPlayerNumber() == 0) {
			this.storage.removeRoom(room_ID);
		}
		CMDummyEvent roomOut = new CMDummyEvent();
		roomOut.setDummyInfo("roomOut#"+ player_ID);
		String session ="session" + (room_ID+1);
		String group = "g1";
		roomOut .setHandlerGroup(group);
		roomOut .setHandlerSession(session);
		m_serverStub.cast(roomOut , session, group);
		roomOut = null;
	}
